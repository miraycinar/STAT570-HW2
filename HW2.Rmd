...
If you want to extract the matches for each group you can use str_match(). 
 This pattern seems to be looking for a string that starts with "the " and is followed by two groups of one or more word characters (denoted by \w+), separated by a space.
...
The output of str_match() (which is a matrix) is converted to a tibble (data frame).
.name_repair = "minimal" is an argument used in as_tibble() to control the way column names are repaired in case they are invalid or duplicated.
set_names("match", "word1", "word2"):
...
The use of (?:) within the regex pattern creates a non-capturing group.
In this case, the non-capturing group (?:e|a) matches either "e" or "a" without capturing this part of the match separately. It's used for grouping without creating a capture group explicitly.
...
```
(skirt|scarf) captures either "skirt" or "scarf".
is matches the word "is".
(?:\\w+) is a non-capturing group (?:) matching any additional word (\\w+ represents one or more word characters).
